...
function renderDayPicker(){
  ...
  /*
   * Bütün ayın məşq statuslarını eyni anda (paralel) yükləyirik,
   * bir-bir gözləmək əvəzinə — beləliklə təqvim bir dəfəyə çəkilir.
   */
  const completedResults=await Promise.all(dayKeys.map(async (currentKey)=>{
    try{
      const workoutResult=await window.storage.get('workout-log:'+currentKey,false);
      return !!(workoutResult && workoutResult.value==='true');
    }catch(e){
      return false;
    }
  }));
  ...
  for(let dayNumber=1;dayNumber<=daysInMonth;dayNumber++){
    ...
    const currentKey=dayKeys[dayNumber-1];
    ...
    if(completedResults[dayNumber-1]){
...
async function renderWorkoutProgress(){
  ...
  updateNarGlow('narGlowProg', count, TARGET_SESSIONS, 'narSvgProg');
